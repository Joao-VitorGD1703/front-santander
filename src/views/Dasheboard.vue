    ...
    <div class="p-6 bg-gray-50 min-h-full  w-full">
      ...
      <div v-if="!dashboardData && !isLoading && !errorMessage" class="text-center py-20">
        ...
        <button @click="triggerFileInput" class="bg-red-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-red-700 transition-colors">
          <svg class="inline-block w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
          Inserir CSV
        </button>
      </div>
      ...
      <div v-if="dashboardData" class="animate-in fade-in duration-500">
        ...
        <div class="bg-white p-4 rounded-lg shadow-md mb-6 flex justify-between items-center">
          <h3 class="text-xl font-bold text-gray-700">{{ dashboardData.nome_empresa }}</h3>
          <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <div v-for="(grafico, index) in dashboardData.graficos" :key="index" class="bg-white p-4 rounded-lg shadow-md">
            ...
            <apexchart
              v-if="grafico.config_apexchart && grafico.config_apexchart.series"
              width="100%"
              height="350"
              :options="grafico.config_apexchart"
              :series="grafico.config_apexchart.series"
            ></apexchart>
          </div>
        </div>
...
const relatorioHtml = computed(() => {
  if (dashboardData.value && dashboardData.value.relatorio_texto) {
    return marked.parse(dashboardData.value.relatorio_texto);
  }
  return '';
});
...
const handleDownloadCSV = () => {
  if (!originalJsonData.value) {
    alert("Não há dados de CSV para baixar.");
    return;
  }
  ...
  const fileName = `${dashboardData.value?.nome_empresa || 'relatorio'}.csv`;
